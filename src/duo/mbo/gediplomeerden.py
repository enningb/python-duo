...
column_mapping = {'PLAATSNAAM': 'Plaatsnaam'
                , 'INSTELLINGSNAAM': 'NaamInstelling'
                , 'BRIN NUMMER': 'Brinnummer'}

def gediplomeerden_mbo_bestand(gediplomeerden_mbo_url = gediplomeerden_mbo_url):
    """Lees bestand in met mbo-gediplomeerden van DUO-site"""
    data = pd.read_csv(gediplomeerden_mbo_url, sep=';', encoding='latin1')
    return data


def tidy_format_gediplomeerden_mbo():
    ...
    """
    gediplomeerden_ruw = gediplomeerden_mbo_bestand()
    ...
    tidy = pd.melt(gediplomeerden_ruw[target_cols]
                , id_vars=id_vars
                , var_name='Variabele'
                , value_name='Gediplomeerden')
